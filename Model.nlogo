...
globals [
  ...
  ;; FOR REPORTING
  conformist-mortality
  contrarian-mortality
]
...
;; Turtle variables
turtles-own [
  p-conform   ; probability that the turtle is a conformist. Otherwise
              ; it is a contrarian.
  conform?   ; 1 if conformist, 0 if it is a contrarian
  action   ; current action: 0 or 1. 1 is adaptive if environment = 1, and
           ; 0 is adaptive if environment = 0.
]
...
to setup
  ...
  ifelse init-prop? = true [
    crt round (population * init-prop-conform) [
      set p-conform 1
      set conform? ifelse-value (random-float 1 < p-conform) [1] [0]
      set action random 2
    ]
    crt round (population * (1 - init-prop-conform)) [
      set p-conform 0
      set conform? ifelse-value (random-float 1 < p-conform) [1] [0]
      set action random 2
    ]
...
;; Procedure each tick.
to go
  ;; Update the environmental conditions according to the
...
;; The turtle samples the actions of other turtles. Then it
;; update its own action based on the actions of others.
to act
  ;; Sample the actions of other turtles. Sum the actions to
  ;; obtain the number of 1s.
  let sample sum (n-of sample-size [action] of turtles)

  ;; Update own action based on the sample and whether this
  ;; agent is a conformist or contrarian.
  if sample > sample-size / 2 [   ; if action 1 is a majority
    ifelse conform? = 1 [   ; if the turtle is a conformist
      set action 1
    ] [   ; if the turtle is a contrarian
      set action 0
    ]
  ]
...
to kill
  ...
  ;; FOR REPORTING. Calculate mortality rate of conformists
  ;; and contrarians by counting each type before and after
  ;; the deaths occur.
  let conformists-before count turtles with [conform? = 1]
  let contrarians-before count turtles with [conform? = 0]
  ...
  ifelse conformists-before > 0 [
    let conformists-after count turtles with [conform? = 1]
    set conformist-mortality 1 - (conformists-after / conformists-before)
  ] [
    set conformist-mortality "NA"
  ]
  ifelse contrarians-before > 0 [
    let contrarians-after count turtles with [conform? = 0]
    set contrarian-mortality 1 - (contrarians-after / contrarians-before)
  ] [
    set contrarian-mortality "NA"
  ]
end
...
;; Constrain a number to a specied interval.
to-report constrain [num lower upper]
  if num < lower [
    report lower
  ]
  if num > upper [
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot mean [conform?] of turtles"
...
940
535
Proportion Action 1 by Social Type over Time
Time
Proportion Action 1
0.0
10.0
...
"" ""
PENS
"Conformists" 1.0 0 -16777216 true "" "plot mean [action] of turtles with [conform? = 1]"
"Contrarians" 1.0 0 -2674135 true "" "plot mean [action] of turtles with [conform? = 0]"
...
1300
535
Mortality Rate by Social Type over Time
Time
Mortality Rate
0.0
10.0
...
"" ""
PENS
"Conformists" 1.0 0 -16777216 true "" "plot conformist-mortality"
"Contrarians" 1.0 0 -2674135 true "" "plot contrarian-mortality"
